    ...
    jQuery.ajax({
        ...
        success: function (response) {
            if (response.code == 'success') {
                ...
                jQuery('#post_image_edit').html('<button type="button" onclick="listfoliopro_remove_post_image(\'post_image_div\');"  class="btn btn-small-ar">X</button>');
                jQuery.colorbox.close();
...
function listfoliopro_remove_topbanner_image(profile_image_id) {
    ...
    jQuery('#post_image_topbaner').html('<button type="button" onclick="listfoliopro_topbanner_image(\'post_image_topbaner\');"  class="btn btn-small-ar">Add</button>');
    ...
    image_gallery_frame.on('select', function () {
        ...
        selection.map(function (attachment) {
            ...
            if (attachment.id) {
                ...
                jQuery('#post_image_topbaner').append('<button type="button" onclick="listfoliopro_remove_topbanner_image(\'post_image_topbaner\');"  class="btn btn-small-ar">X</button>');
...
function listfoliopro_edit_gallery_image(profile_image_id) {
    ...
    image_gallery_frame.on('select', function () {
        ...
        selection.map(function (attachment) {
            attachment = attachment.toJSON();
            if (attachment.id) {
                jQuery('#' + profile_image_id).append('<div id="gallery_image_div' + attachment.id + '" class="col-md-3"><img  class="img-responsive"  src="' + attachment.url + '"><button type="button" onclick="listfoliopro_remove_gallery_image(\'gallery_image_div' + attachment.id + '\', ' + attachment.id + ');"  class="btn btn-small-ar">X</button> </div>');
...
function listfoliopro_attached_doc(profile_image_id) {
    ...
    image_gallery_frame.on('select', function () {
        ...
        selection.map(function (attachment) {
            attachment = attachment.toJSON();
            if (attachment.id) {
                jQuery('#' + profile_image_id).append('<div id="attached_div' + attachment.id + '" class="row mb-2"><label class="col-md-11 control-label">' + attachment.title + ' </label><div class="col-md-1"><button type="button" onclick="listfoliopro_remove_attached_doc(\'attached_div' + attachment.id + '\', ' + attachment.id + ');"  class="btn btn-small-ar">X</button></div></div>');
