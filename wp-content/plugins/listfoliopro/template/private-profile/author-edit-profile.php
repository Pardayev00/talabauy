...
<div class="row"> 
	...
	<div class="col-md-12">
		...
		<?php esc_html_e('Change Logo','listfoliopro'); ?> </button>
	</div>
	...
	<div class="col-md-12 mt-3">
		<div class="form-group">
			<label class=""><?php esc_html_e('Industry', 'listfoliopro'); ?></label>
	...
	<div class="col-md-12 mt-3">
		<div class="form-group">		
			<label class=""><?php esc_html_e('Location', 'listfoliopro'); ?></label>
			<hr/>
			<div class=" row">		
			<?php
				$args =array();					
			...
			?>
				<div class="col-md-12">
					<input type="text" class="form-control" name="new_location" id="new_location" value="<?php echo get_user_meta($current_user->ID,"new_locations",true); ?>" placeholder="<?php  esc_html_e('Enter New Locations: Separate with commas','listfoliopro'); ?>">
				</div>	
		</div>
		</div>
	</div>	
		...
		foreach ( $default_fields as $field_key => $field_value ) { 		
			if(isset($myaccount_fields_array[$field_key])){  				
				...
				if($myaccount_fields_array[$field_key]=='yes'){ 
					...
					if($role_access=='yes'){
						echo  $main_class->listfoliopro_check_field_input_access($field_key, $field_value, 'myaccount', $current_user->ID );
					}
				}
				}else{ 
				echo  $main_class->listfoliopro_check_field_input_access($field_key, $field_value, 'myaccount', $current_user->ID );
