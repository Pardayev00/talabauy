...
class Listfoliopro_Accordion_Widget extends Widget_Base{

	public function get_name(){

		return "listfoliopro_accordion";
	}
	public function get_title(){
		return esc_html__("Accordion","listfoliopro");
	}
	...
	protected function _register_controls(){
		$this->start_controls_section(
			...
			[
				'label' => esc_html__( 'Accordion', 'listfoliopro' ),
		...
		$repeater->add_control(
			'accordion_title',
			[
				'label'       => esc_html__( 'Accordion Title', 'listfoliopro' ),
				'type'        => Controls_Manager::TEXTAREA,
				'rows'        => 5,
				'default' => 'Accordion Title Here',
				'placeholder' => esc_html__( 'Type Accordion Title Here', 'listfoliopro' ),
			]
		);

		$repeater->add_control(
			'accordion_content', [
				'label' => esc_html__( 'Accordion Content', 'listfoliopro' ),
		...
		$this->add_control(
			'accordion_list',
			[
				'label' => __( 'Accordion Lists', 'listfoliopro' ),
		...
		$this->add_control(
			'open_by_default',
			[
				'label'       => esc_html__('Open By Default', 'listfoliopro'),
				...
				'default'     => 1,
				'separator'     => 'before',
				'description' => esc_html__('Which accordion you want to open by default.', 'listfoliopro'),
			]
		);
		...
		$this->add_group_control(
			...
			[
				'name' => 'title_typo',
				'label' => __( 'Title Typography', 'listfoliopro' ),
		...
		$this->add_control(
			'title_color',
			[
				'label'       => esc_html__('Title Color', 'listfoliopro'),
		...
		$this->add_control(
			'title_hover_color',
			[
				'label'       => esc_html__('Title Active Color', 'listfoliopro'),
				'type'        => Controls_Manager::COLOR,
				'selectors'   => [
					'{{WRAPPER}} .ep-accordion-wrapper .accordion-button:hover,
					{{WRAPPER}} .ep-accordion-wrapper .accordion-button:not(.collapsed)' => 'color: {{VALUE}};',
				],
			]
		);
	...
	protected function render() {
		...
		$id = rand(100, 100000);
		?>
        <div class="ep-accordion-wrapper ep-accordion-wrapper-<?php echo esc_attr($id);?>">
...
Plugin::instance()->widgets_manager->register( new Listfoliopro_Accordion_Widget );