...
<div class="sidebar-border">
	<div class="toptitle mb-3"><?php esc_html_e('Recent listing', 'listfoliopro'); ?></div>
	<?php
		if ( ! empty( $listfoliopro_similar ) ) {
			$i=0;$company_logo_sim='';
			foreach( $listfoliopro_similar as $listing_sm ){
				$company_logo_sim='';
				...
				if($listing_contact_source_sim=='new_value'){
					...
					$company_logo_sim=get_user_meta($author_id_sim, 'listfoliopro_profile_pic_thum',true);
				}
			...
			<div class="row  card-list-4  " >
				...
				<div class="col-md-9 col-9 mt-3">
					...
					<div class="mt-0 "><span class="card-time"><span> 
							<?php
							$currentCategory_sl = $main_class->listfoliopro_get_categories_caching($listing_sm->ID,$listfoliopro_directory_url);
							$saved_icon_sl='';
							$cat_name2_sl='';
							$iii=0;
							if(isset($currentCategory_sl[0]->slug)){										
								foreach($currentCategory_sl as $c){							
									if(trim($saved_icon_sl)==''){
										$saved_icon_sl= listfoliopro_get_cat_icon($c->term_id);
									}
									if($iii==0){
										$cat_name2_sl = $c->name;
										}else{
						...
						?><i class="ml-1 <?php echo esc_html($saved_icon_sl); ?>"></i><?php echo esc_html($cat_name2); ?></span></span></div>		  
						<div class="row mt-2 ">							
							<?php
								$location_array= wp_get_object_terms( $listing_sm->ID,  $listfoliopro_directory_url.'-locations');
