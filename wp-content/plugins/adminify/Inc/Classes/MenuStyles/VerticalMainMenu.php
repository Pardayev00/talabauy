...
?>
		<div class="wp_adminify_sidebar_admin-menu menu-default-light">
			<ul class="wp_adminify_admin-menu">
				...
				$this->render_top_level_menu_items($this->original_menu);
		...
		foreach ($the_menu as $menu_item) {
			...
			if (strpos($custom_menu, 'adminify-custom-menu-') !== false) {
				if (isset($menu_options[$menu_item[5]]['hidden_for'])) {
					$hidden_for = $menu_options[$menu_item[5]]['hidden_for'];
					if (!empty($hidden_for)) {

						$disabled_for = $this->jltwp_adminify_menu_roles($hidden_for);
						if (Utils::restricted_for($disabled_for)) {
							continue;
						}
					}
			...
			// Amelia Menu issue
			if ('admin.php?page=amelia' === $link) {
				$link = 'admin.php?page=wpamelia-dashboard';
			}
	...
	{
		$sub_menu_setting = isset($menu_options[$parent_menu[2]]['submenu']) ? $menu_options[$parent_menu[2]]['submenu'] : array();

		?>
		<ul class="wp-adminify-nav-sub wp-submenu wp-submenu-wrap">
			<?php
			foreach ($sub_menu as $sub_item) {
				$restrict_submenu = false;
				...
				if (strpos($parent_menu[5], 'adminify-custom-menu-') !== false) {
					if (strpos($custom_submenu, 'adminify-custom-submenu-') !== false) {
						...
						$hidden_for = isset($submenu_options['hidden_for']) ? $submenu_options['hidden_for'] : array();
					...
					$disabled_for[] = !empty($sub_menu_setting[$sub_item[2]]['hidden_for']) ? $this->jltwp_adminify_menu_roles($sub_menu_setting[$sub_item[2]]['hidden_for']) : array();
				}
				...
				if (($sub_menu_link === 'wp-adminify-settings-pricing') && jltwp_adminify()->can_use_premium_code__premium_only()) {
					...
					if (!empty($disabled_for)) {
						$restrict_submenu = Utils::restricted_for($disabled_for);
